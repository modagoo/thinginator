...
class Thing < ActiveRecord::Base
  ...
  def render_thing(thing, p)
    ...
    when "File"
      if thing.send(p.slug.to_sym).present?
        "#{thing.send(p.slug.to_sym).original_filename} https://tufnell.essex.ac.uk#{thing.send(p.slug.to_sym).to_s}"
      else
        "No file"
      end
    when "Markdown"
  ...
  def get_value(property)
    ...
    if existing_value.nil?
      return self.content.includes(:contentable).where(property: property).first.try(:contentable).try(:value)
    else
